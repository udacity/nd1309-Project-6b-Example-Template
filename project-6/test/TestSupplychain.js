...
contract("SupplyChain", function (accounts) {
  ...
  it("Testing smart contract function buyItem() that allows a distributor to buy coffee", async () => {
    ...
    const expectedFarmerBalance =
      Number(initialFarmerBalance) + Number(productPrice);
    const expectedDistributorBalance =
      Number(initialDistributorBalance) - Number(productPrice);
    ...
    const newDistributorBalance = await web3.eth.getBalance(distributorID);
    ...
    assert.equal(eventEmitted, true, "Invalid event emitted");
    assert.equal(
      Number(newFarmerBalance),
      expectedFarmerBalance,
      "Invalid farmer balance"
    );
    assert.equal(
      Number(newDistributorBalance),
      expectedDistributorBalance,
